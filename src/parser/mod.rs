...
impl<'a> Parser<'a> {
    ...
    pub fn parse_drop(&mut self) -> Result<Statement, ParserError> {
        ...
        let object_type = if self.parse_keyword(Keyword::TABLE) {
            ...
            return self.expected_ref(
                "AI MODEL, COLLATION, CONNECTOR, DATABASE, EXTENSION, FUNCTION, INDEX, OPERATOR, POLICY, PROCEDURE, ROLE, SCHEMA, SECRET, SEQUENCE, STAGE, TABLE, TRIGGER, TYPE, VIEW, MATERIALIZED VIEW or USER after DROP",
                self.peek_token_ref(),
            );
        };
